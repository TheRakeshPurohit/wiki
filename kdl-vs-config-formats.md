...
> **集合优先（单数是特例）**：语法不区分单复数，正是书写端要主动确立的默认纪律——一个节点默认按「可承载 N 个同类」设计，单个是特例（初版只有一个 `extractor` 也应写成复数包裹 `extractors {}`）。理由：集合对新增闭合，加成员是局部、非破坏操作；单槽位扩多槽才需要动 schema。此原则与 Fractal 宪章 §3 一致。
