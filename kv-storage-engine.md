...
- **分布式 → 元数据走共识，数据走存算分离**：共识（Openraft）只承载**元数据规模**协调（登录态、Catalog 指针、配置），不适合大规模数据同步——Raft 不分片、每节点为全量副本，存储与写放大随节点数线性增长（3 副本的 3x 可接受但无法随节点扩展）。大规模数据走 SlateDB + S3（存算分离，副本由 S3 3-AZ 兜底）或 Fjall 写本地 + Lakehouse 落湖。
...
| 本文档论点 | OpenAI 实践验证 |
|:--|:--|
| 查询模式可预测时 KV 优于 SQL（§6.5） | 核心流量 = 纯 KV 点查 + 前缀扫描 |
