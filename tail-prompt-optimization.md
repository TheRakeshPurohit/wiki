...
传统压缩声称"辅助模型比 cache 便宜"，但分摊到每次调用后收益消失：

- 辅助模型价格 ≈ 主模型的 1/60，cache 价格 ≈ 主模型的 1/10
- 假设 100 轮压缩一次，压缩模型省下 (1/10 - 1/60) = 8.3% 的 input 成本
- 分摊到每次调用：8.3% / 100 = **0.083%**
- 代价：压缩质量损失（信息丢失、压缩器缺乏 Agent 上下文）

尾提示词本身也是压缩，但质量更高——它利用 Agent 正常 turn 的注意力完成压缩，而非独立调用。没有尾提示词时，直接用高级模型处理全量上下文速度慢、价格贵；尾提示词让高级模型在正常 turn 中顺带完成压缩，解决了这个问题。

辅助模型虽然单 token 速度快，但无 KV cache（命中率 0%），必须重新计算全量 input。主模型 + 尾提示词的历史部分走 cache（命中率 ~99%），实际延迟可能更低。
