...
多个 turn 共享同一段上下文缓存（Context Cache）前缀，形成树状结构：
...
核心特性：
- **共享 trunk**：所有 branch 复用父节点的上下文缓存，不重复计算
...
*注入 prompt 末尾的一段指令，利用上下文缓存的旁路分支机制完成特定任务。*
...
类比 TCO（尾调用优化）：尾调用复用当前栈帧，尾提示词复用当前上下文缓存。都是"尾部"操作，都为了复用已有状态。

### 核心特性

- **利用 cache**：历史部分走上下文缓存，只有尾提示词是新计算
...
| 上下文缓存（Context Cache） | 从零计算（命中率 0%） | 历史部分走 cache（命中率 ~99%） |
...
辅助模型虽然单 token 速度快，但无上下文缓存（命中率 0%），必须重新计算全量 input。主模型 + 尾提示词的历史部分走 cache（命中率 ~99%），实际延迟可能更低。
