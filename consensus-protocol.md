...
## 7. 实现视角：reducer、日志仓库与快照压缩

前几节讲 Raft 的机制与边界；本节从"实现一个 Raft 应用（如 Openraft 库）"的角度，说明 reducer（命令还原）、日志仓库、快照压缩如何分工，以及使用者要写什么、不用写什么。为便于理解，用一个最简单的 KV 命令集举例：业务命令为 `Put(key, value)` 与 `Delete(key)`，状态为 `HashMap<key, value>`。

### 7.1 日志 = 命令 + 一致性元数据

日志条目不是"只装命令"。每条 entry 包含：

| 字段 | 作用 |
|:---|:---|
| `log_id`（term + leader 节点 + index） | "这条命令挂在哪个任期、哪个位置"的印章，是选举与日志匹配规则的基础 |
| `context`（可选） | 实现库的扩展字段，一般为空 |
| `payload`（命令） | 真正的业务操作，如 `Put("k", "v")` |

日志仓库里还存两块**不属于业务命令**的东西：
- 硬状态：`current_term` + `voted_for`（当前任期、本届投给谁）——防脑裂重复投票；
- 边界记账：`last_log_id` / `last_purged_log_id`（日志从哪读到哪、已压缩到哪）。

只有 `payload` 是业务命令；其余都是让所有节点"对同一串命令、同一顺序、同一任期"达成共识所需的一致性元数据。

### 7.2 术语：RSM 是自动机，工程语境不叫"状态机"

复制状态机的 `apply`（reducer）是一个（大状态空间、可无限）确定性自动机的转移函数 `δ`——形式上是自动机。但"状态机"一词有**两种语域**，需分开判：

- **形式语域**：自动机 = 状态机，同一五元组 `M = (Q, Σ, δ, q0, F)`，`δ` 就是 reducer，`Q` 可无限（下推/图灵皆然）——web、raft 都是自动机。
- **工程语域**（"状态机模式" State pattern、状态机类库）：只指"命名状态 + 按状态限制操作 + 跳转"的那类工作流 FSM。RSM 没有命名状态、没有"此状态只能做哪些操作"的转移矩阵，**因此在此语域不是"状态机"，用 reducer 更贴切**。

两语域分别成立：形式上是自动机（别用形式否认工程窄义），工程语域才谈得上"算不算状态机"（别用工程窄义否认形式涵盖）。完整辨析见 [自动机、状态机与 reducer](automaton-vs-state-machine.md)。

```
RaftLogStorage    ← 盲存盲读：把 entry 当"带 log_id 的 blob"存/取，不反序列化命令
reducer(apply)    ← 真懂数据：反序列化命令跑状态迁移（＝大状态自动机的 δ）；快照序列化业务状态
```

- **日志仓库是哑持久层**：引擎传入原始条目，使用者只管落盘、按 range 原样返回，不要在这里解析命令。
- **reducer 是业务落点**：`apply` 把已提交命令按日志 index 顺序折叠进状态，是确定性的核心——同一串命令、同一初始状态，结果必唯一。

### 7.3 快照与日志压缩

日志只增不减，若不压缩会无限膨胀，故需快照压缩：
...
要点：
- **快照是业务数据的固化**（结果状态），不是命令的副本；日志是命令（怎么来的），快照是状态（现在是多少）。
- `purge_logs` 的触发点是**快照点 last_applied**，不是"所有节点达成一致"。quorum 决定"哪些被提交"，压缩决定"日志删到哪"，两者正交。
- 删日志前缀是安全的，因为**快照接管了那一段**：谁需要这段就发快照兜底。
- 代价：删掉前缀后失去命令级回放/审计史，只剩结果快照。要审计就得另存命令（如 S3 归档）。

这与**事件溯源**同构：日志 = 事件流，快照 = 投影；快照只是为压缩而打的投影。

### 7.4 使用者要写什么，不用写什么

| 你要写的 | 性质 |
|:---|:---|
| 类型配置（NodeId / Entry / 快照类型） | 纯类型胶水，无逻辑 |
| 日志仓库（存 entry + 硬状态 + 记账） | 哑持久化，盲存盲读 |
| reducer `apply` | **业务核心**：反序列化命令 → 状态迁移 |
| 状态机快照 `build / install_snapshot` | 序列化 / 反序列化业务状态 |
| 网络传输（把引擎消息包成 RPC） | 机械接线 |

| 你完全不用管（引擎自动） | |
|:---|:---|
| term / index / leader 选举 / 投票 / quorum | 引擎内部决策 |
| 日志复制 / 提交推进 / follower 追平 / 快照触发节奏 | 引擎自动跑 |

一句话：共识不用写，你只是给引擎"一个能存日志的仓 + 一个会 apply 的 reducer"。真正要"懂"数据的只有两处——`apply`（跑命令）与快照（读写业务状态）；日志仓库与网络是盲存盲读的机械接线。

### 7.5 快照删除的安全性

快照压缩的安全性由这条链兜住：**已提交 → 已 apply → 已被快照覆盖 → 才可 purge**。任何未提交 / 未 apply 的 index 都不得删除。
