...
**用户方案：** 在已有对话末尾追加压缩指令，复用上下文缓存（Context Cache）让主模型自摘要，生成后用摘要替换历史，压缩指令不入历史。核心价值在于缓存复用。
...
| 5 | 再问：用得多吗？ | 区分：自摘要大模式常见，但精确变体（末尾追加+复用上下文缓存+指令不入历史）在大厂少见；给出三个"硬约束"（有损累积、跨会话不可用、缓存非100%可靠） | AI 再次寻找反驳点 |
...
| 回合 | 用户 | AI | 结果 |
|------|------|-----|------|
| 14 | Hermes 用辅助模型，不同模型能借缓存吗？辅助模型有任何优势吗？ | 确认不同模型绝对不能共享上下文缓存（Context Cache）；辅助模型在缓存维度零优势，仅在 API 计费/显存隔离场景有妥协价值 | **用户击穿辅助模型路线** |
...
```mermaid
graph TD
    A["用户提出方案<br/>末尾追加指令+复用上下文缓存"] --> B["AI: 列优势但抛挑战"]
