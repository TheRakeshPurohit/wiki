...
- **集中式服务形态（路径 A 独有，相对路径 B 的嵌入式是实打实的优势）**：PostgreSQL 以独立服务形态部署，单一实例可服务多个项目（资源池化、复用连接与缓冲），可上云托管（RDS/CloudSQL 免自运维），应用层保持无状态（存储外置为唯一持久点）。这三条红利**来自「集中式 vs 嵌入式」的部署形态，不是 SQL 语言性质**——若路径 B 想拿同样红利，正解是换**集中式 KV server**——即 KV 引擎 + HTTP/WS 网络访问层的组合（内部 WS+Postcard、对外 HTTP 兜底，见 [KV 存储引擎](kv-storage-engine.md)「KV server 的构成」），而非回到 SQL。此处把它们记为路径 A 的优势，正因为路径 A 恰好以 PostgreSQL=集中式形态落地；论断源于集中式属性，只是落在 PostgreSQL 头上（归因辨别见 [KV 存储引擎](kv-storage-engine.md)「资源池化 / 云托管 / 无状态：是拓扑性质，不是 SQL 的性质」）。
