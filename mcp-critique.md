...
#### 渐进披露的层级：付费建造 vs 免费继承

按需检索的优势之上还有一层更深的差距：**实现渐进披露的成本与深度**。

MCP 的渐进披露要逐级建造接口。要一层「详情」，得给 tool 加一个详情接口；要一层「参考」，再加一个参考接口——协议没有层级导航原语，每一级都是人为设计的 RPC，schema 逐级膨胀、每级都要维护。MCP 不但实现上不灵活，这也不是人类使用工具的习惯——靠针对 MCP 用法做模型特化训练来弥补，是逆着训练数据分布走，效果存疑。

Skill 的多级披露（简介 → 详情 → 参考）里，只有简介需要额外处理（索引或 frontmatter），往下全是文件系统导航：目录结构即层级，读取即下钻。内容组织形式与产品手册同构——README → 章节 → 附录，LLM 的训练语料里有海量这类文档，这套导航模式已完全掌握。Skill 的渐进披露是搭在模型已有能力上的免费机制；MCP 的按需加载是为模型新造的 API 面，逐级付费。
...
### 有状态上游的正确姿势：薄层转发，不是影子服务

MCP server 面对一个有状态的上游 API（GitHub、Linear、数据库服务）时只有两种姿势：

- **薄层**：只转发不理解。请求原样透传，Token 原样携带，上游的鉴权、权限、状态全部留在上游。这是正确的策略——但它恰恰不需要常驻有状态的协议形态，一个随用随走的 CLI 直连 HTTPS 就是最薄的层。
- **影子服务**：在本地重实现上游的一部分。多步操作要建本地会话/账户体系，上游的功能被复制到 MCP server 内部维护——中央体系下的联邦模式，比上游和薄层任何单一形态都差：一致性无法保证（上游演进，影子滞后），状态两处存储，故障点翻倍。

MCP 的常驻 server 形态天然把实现者往影子服务推：连接已经建立、状态已经在内存里、顺手缓存一点「优化体验」的诱惑无处不在。而薄层转发这个唯一正确的策略，用 CLI 每次冷启直连反而更自然——无状态进程想藏状态都没地方藏。协议形态与正确策略错位，这是比「笨重」更深一层的批评。
...
| **披露层级** | 逐级建造接口（详情/参考各一个 RPC） | 文件系统导航（目录即层级，免费继承） |
| **上游集成** | 影子服务诱惑（常驻形态藏状态） | 薄层转发（无状态进程无从藏状态） |
| **工程质量** | 无规范约束，靠实现者自觉 | CLI 规范内化报错/日志实践 |
| **生态兼容** | ✅ 跨客户端标准 | ❌ 每框架自行实现 |
...
### 管控对等性：文件系统 ACL 即企业管控面

「MCP 的网络边界提供更佳管控力」在企业治理视角下不成立。企业管控关心的是审计、鉴权与资产合规，两条路线的控制面对等：

- MCP：中央注册表登记 server 的 URL 或 stdio 启动路径，网关截获 JSON-RPC 流量审计。
- Skill：统一存放于受控目录树（中央私有 Git 仓/包管理器），框架拉起子进程时拦截 stdout/stderr 与系统调用做全量日志。

文件系统 ACL（用户组权限、目录读写限制）是历经数十年考验、成熟且低心智负担的管控技术。Skill 不是散落的「野脚本」，而是与 MCP server 同等受控的企业静态资产——协议网关能做到的审计粒度，进程级拦截 + 目录权限同样能做到。
...
报错质量、结构化日志这类「实现细节」常被当作协议中立因素——两者都可以做好，所以不算差异。这个推理漏掉了**责任落在谁头上**：没有规范强制时，质量靠每个实现者的自觉，而自觉的覆盖率取决于实现成本与动力的对比。

Skill 的规范天然把工程实践内化：写一个给人用的 CLI，报错信息本来就是接口的一部分（`Error: missing option '--repo'` 直接可读可修），typer 这类框架免费给出结构化帮助；日志用 structlog 沉淀为结构化事件，duckdb 一条 SQL 就能分析排查。这些是随用随走进程的自然写法——进程跑完即退，所有信息必须当场落到输出里，否则就没了。

MCP server 是常驻服务，工程责任反而更重——运维、监控、进程守护、依赖管理全是额外故障点——但协议对这些没有任何约束，报错被包装成 JSON 错误码的形态也切断了 LLM 用自然语言纠错的通道。质量取决于实现者，规范不强制，运维复杂度还放大了欠账的代价。协议中立的说法只看到了「都能做好」，没看到「一个的规范让做好是默认，另一个的规范让做好是自觉」。
