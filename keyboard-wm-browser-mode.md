...
**硬约束 · 几乎 niri 独享**：整套模式依赖两个**结构性**条件——① WM 提供**接口/CLI 级精细控制**（窗口移动 / 聚焦 / 设列宽 / workspace 路由等 IPC 动作）；② 该 WM 的 **workspace 模型**适合「一 session 一 workspace」。

- **IPC 层面**：niri、hyprland 满足；cosmic-de 目前似乎还不满足（窗口层控制能力不足）。
- **但仅 IPC 不够，workspace 模型才是分水岭**：
  - **平铺类（如 hyprland）不合适**：workspace 有**上限**，单个 workspace 内能排布的窗口数也有限——实际限制了「会话数任意增长 + 每会话多窗口平铺」的形态。
